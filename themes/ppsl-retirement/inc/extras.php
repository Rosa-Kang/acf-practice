 ...
 */ 
function wcd_post_columns_data( $column, $post_id ) {
	switch ( $column ) {

	case 'modified':
		$m_orig		= get_post_field( 'post_modified', $post_id, 'raw' );
		$m_stamp	= strtotime( $m_orig );
    $modified	= date('Y/m/d @ g:i a', $m_stamp );
...
function wcd_post_columns_display( $columns ) {
	$columns['modified']	= 'Last Modified';

	return $columns;
}

add_action( 'manage_posts_custom_column', 'wcd_post_columns_data',	10,	2	);
add_filter( 'manage_edit-post_columns', 'wcd_post_columns_display' );

// Filter CPT with last modified date
add_filter( 'manage_edit-jobs_columns', 'wcd_post_columns_display' );
add_filter( 'manage_edit-team_members_columns', 'wcd_post_columns_display' );
add_filter( 'manage_edit-faqs_columns', 'wcd_post_columns_display' );
add_filter( 'manage_edit-suites_columns', 'wcd_post_columns_display' );
add_filter( 'manage_edit-testimonials_columns', 'wcd_post_columns_display' );
add_filter( 'manage_edit-business_info_columns', 'wcd_post_columns_display' );


/**
 * Display last modified date on pages
 */
function wcd_heirch_columns( $column, $post_id ) {
	switch ( $column ) {
...
function wcd_page_columns( $columns ) {
	$columns['modified']	= 'Last Modified';

	return $columns;
}

add_action( 'manage_pages_custom_column', 'wcd_heirch_columns',	10,	2	);
add_filter( 'manage_edit-page_columns', 'wcd_page_columns' );